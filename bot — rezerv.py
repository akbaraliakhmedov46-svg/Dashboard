...
API_TOKEN = os.getenv("BOT_TOKEN", "7980938543:AAEa__0yuK3it9_k9J9uikz9ESHKPFHrqNs")
GOOGLE_KEY_FILE = os.getenv("GOOGLE_KEY_FILE", "eastern-clock-469408-n2-b342542841f7.json")
...
WORKING_DAYS_IN_MONTH = int(os.getenv("WORKING_DAYS_IN_MONTH", "26"))
...
def create_enhanced_workdays_calendar(ax_calendar, monthly_data):
...
def create_production_dashboard():
    ...
    try:
        ...
        # 1. ИШ КУНЛАРИ ЖАДВАЛИ (2 устунли) - ТЕПАДА
        ax_calendar = fig.add_subplot(gs[0, :])
        create_enhanced_workdays_calendar(ax_calendar, monthly_data)
...
# ------------------- MAIN -------------------
async def main():
    logger.info("🚀 Бот ишга туширилмоқда...")
    
    try:
        await dp.start_polling(bot)
    except Exception as e:
        logger.error(f"❌ Ботни ишга туширишда хato: {e}")
    finally:
